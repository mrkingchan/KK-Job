...
- (void)czh_setView {
    ...
    datePickerView.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"zh-CN"];
...
- (void)czh_setDate {
    ...
    NSArray * arr = [[UtilityHelper getCurrentTimes] componentsSeparatedByString:@"-"];
    NSString * string = [NSString stringWithFormat:@"%zd-%@-%@",[arr[0] integerValue] - 15,arr[1],arr[2]];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:FORMAT];
    NSDate * maxDate =  [formatter dateFromString:string];
    ...
    if (self.currentDate.length <=0) {//如果没有初始时间用当前时间
        ...
        NSDate *date = [formatter dateFromString:self.currentDate];
